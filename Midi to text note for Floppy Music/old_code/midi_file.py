...
for message in MidiFile('tetris.mid'):
    #time.sleep(message.time)
    if not isinstance(message, MetaMessage):
        try:
        	if message.time != 0:
        		if re.sub("\D", "", notes_number[message.note]) > 3:
        			print "NOTE_%s%s, %s," % (re.sub("[0-9]+", "", notes_number[message.note]).replace("#", "S"), int(re.sub("\D", "", notes_number[message.note]))-4, int(message.time*10)+4)
        		else:
        			print "NOTE_%s, %s," % (notes_number[message.note].replace("#", "S"), int(message.time*10)+4)
        	else:
        		#print "0, 2,"
        		pass
        except:
